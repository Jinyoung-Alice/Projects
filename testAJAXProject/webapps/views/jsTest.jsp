      ...
      <h1>자바 스크립트와 Ajax</h1>
       ...
       <script>
             ...
             function callbackFn() {
            	 ...
            	 // 3-2. 요청 정보를 설정 
            	 httpRequest.open("GET", "/ajax/test1.do?name=한진영", true);
       ...
       function test2() {
    	   ...
    	   // 2. 요청 정보 설정 
    	   httpRequest.open("GET", "/ajax/test2.do?name=한지민", true);
       ...
       function result2() {
    	   ...
    	   if(httpRequest.readyState == 1) {
    		   //alert("open 함수는 호출되었지만, 아직 send는 안 한 상태");
    	   } else if (httpRequest.readyState == 2) {
    		   //alert("send 함수가 실행되며 어떠한 경로로 연결할 지, 기본 정보만 전달한 형태");
    	   } else if(httpRequest.readyState == 3) {
    		   //alert("send 함수가 데이터가 일부 수신된 상태");
    	   } else if(httpRequest.readyState == 4) {
    		   //alert("데이터 전송 완료");
    		   
    		     if(httpRequest.status == 200) { // 전송ok (배송 완료)
    		    	  alert(httpRequest.responseText);    		    	 
    		     } else {
