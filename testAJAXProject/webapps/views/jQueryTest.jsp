...
<body>
	...
	$.ajax({
		...
		type : 'get'|'post',
		data : '전달할 값' / 10,
		     // 만약 여러 개라면 { name : '홍길동', age : 10, hobby : 구슬치기 },
		async : true | false, (비동기 | 동기),
	...
	<script>
		$('#getResult').on('click', function(){
			$.ajax({
				...
				success : function(data){  // data 대신 다른 이름을 써도 상관 X
					alert(data);
	...
	<p>
		...
		이름 : <input type="text" id="userNick" /> <br>
	...
	<script>
		...
		$('#testBtn6').on('click', function(){
			$.ajax({
				url : '/ajax/test6.do',
				type : 'get',   // 만약 type 을 생략하면 기본은 get 방식
				data : { pnoList : $('#testPno').val() },
				success : function(data) {
					...
					for(var i = 0 ; i < data.list.length ; i++) {
						...
						var $pno = $('<td>').text(product.pno); // <td>1</td> 형식으로 만들어짐
	...
	<h3> 8. Gson을 이용한 List 처리하기</h3>
	
	<button id="testBtn8">실행 확인</button>
	&nbsp;
	<select id="selectGson"></select>
